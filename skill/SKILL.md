...
- （2026-08-31）**#43 沙箱單的絆線**。遇到下列任一情況，先停下來提醒使用者
  「#43 的啟動條件成立了，考慮先做牢房再繼續」，不要默默做下去：
  單子要處理**外部內容**（依賴升級、抓第三方程式碼進來分析）；發現**非使用者本人**
  開的單或留言進了 backlog；或發生**波及目標 repo 以外**的失誤。
  為什麼：無頭 claude 直跑宿主機、摸得到一切；風險評估（2026-08-28）判定當時
  輸入面全是自己人所以不蓋牢房，但把「何時該蓋」寫成這三條。方案全文在 #43。
