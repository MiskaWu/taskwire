...
export const Badge = ({ k = 'idle', children }) => <span className={`badge b-${k}`}>{children}</span>;
export const Btn = ({ kind = '', children, ...rest }) => (
  <button className={`btn ${kind}`} {...rest}>{children}</button>
);
export const Dot = ({ c }) => <span className="dot" style={{ background: c }} />;
...
export const Icon = ({ name, size = 18 }) => {
  ...
  return <svg viewBox="0 0 24 24" width={size} height={size} {...P}>{body}</svg>;
};

export const days = (ts) => (ts ? Math.floor((Date.now() - new Date(ts)) / 86400000) : '');
