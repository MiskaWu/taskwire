...
const strong = (s) => s.split('**').map((seg, i) => i % 2 ? <strong key={i}>{seg}</strong> : seg);

function SettingsForm({ st, reload }) {
  ...
  const save = async () => {
    setMsg({ busy: true });
    const r = await api('/api/config', { values: vals });
    setMsg(r);
    if (r.ok) reload();
  };
  const srcBadge = { env: ['warn', 'env 蓋住'], file: ['accent', '設定檔'], default: ['idle', '內建預設'] };
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', paddingTop: 12, flexWrap: 'wrap' }}>
      ...
      {msg && !msg.busy && (msg.ok ? (
        <div className="okmsg" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          已寫入設定檔
          {msg.shadowed?.length > 0 && <span>⚠ {msg.shadowed.join('、')} 被環境變數蓋住，改了不生效</span>}
...
function Secrets({ st, reload }) {
  const [inputs, setInputs] = useState({});
  const [loaded, setLoaded] = useState({});
  const [msg, setMsg] = useState({});
  if (!st?.ok) return <Spin />;

  const put = (name, r) => setMsg((m) => ({ ...m, [name]: r }));
  const reveal = async (name) => {
    const r = await api(`/api/secret?name=${encodeURIComponent(name)}`);
...
function Notify({ st, reload }) {
  ...
  const send = async () => {
    setOut({ busy: true });
    setOut(await api('/api/notify-test', { kind, message: text }));
    reload();
  };
  const drop = async (key) => { await api('/api/threads/reset', { key }); reload(); };
    ...
    {Object.keys(threads).length === 0
      ...
      : Object.entries(threads).map(([k, v]) => (
        <div key={k} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 72px', gap: 10, alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--line2)' }}>
          <span>{k}</span>
          <span className="mono" style={{ fontSize: 11.5, color: 'var(--ink2)' }}>{v}</span>
...
function SkillHints() {
  ...
  const [msg, setMsg] = useState(null);
  useEffect(() => { api('/api/skill').then((r) => setBody(r.ok ? r.body : r)); }, []);
  if (body === null) return <Spin />;
  if (typeof body === 'object') return <ErrBox r={body} />;
  const save = async () => {
    setMsg(await api('/api/skill', { body }));
...
export default function Settings({ st, reload }) {
  return (<>
    <Topbar title="設定與密鑰"
