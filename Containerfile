...
# 少給的部分不會壞掉，只會降級成在頁面上把該跑的指令印給你複製（見 host_action）。
#
# 裝的東西分三層，愈往下愈是為了容器化本身而付的代價：
#   1. python3        —— 控制台本體。只用標準函式庫，沒有 pip 相依。
#   2. bash jq curl   —— task／task-notify／task-doctor 這些腳本要用。
#   3. glab, systemd  —— 查單況要 glab；按得動「重啟門鈴」要 systemctl 與 journalctl，
#                        而它們只存在於 systemd 套件裡。第 3 層是純粹的容器稅：
#                        直跑的話這兩樣宿主機上本來就有。
...
RUN apt-get update && apt-get install -y --no-install-recommends \
      python3 bash jq curl ca-certificates \
...
# 第 3 層：只為了讓頁面上的「重啟／停止／看 journal」真的按得動。
